...
function signToken(payload: { userId: string; role: UserRole }): string {
  ...
  const sig = crypto.createHmac('sha256', JWT_SECRET).update(b64(header) + '.' + b64(body)).digest('base64url');
...
function verifyToken(token: string): AuthPayload | null {
  try {
    ...
    const expected = crypto.createHmac('sha256', JWT_SECRET).update(hB64 + '.' + bB64).digest('base64url');
...
app.get('/api/appointments/me', requireAuth('client'), (req, res) => {
  try {
    const auth = (req as express.Request & { auth: AuthPayload }).auth;
    const rows = stmts.getAppointmentsByUser.all(auth.userId) as AppointmentRow[];
